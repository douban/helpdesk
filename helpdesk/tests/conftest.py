...
from helpdesk.views.api.schemas import (
    ApproverType,
    GroupUserReq,
    Node,
    NodeDefinition,
...
async def test_client():
    ...
    async with AsyncClient(
        ...
        base_url="http://test",
        headers={"host": config.TRUSTED_HOSTS[0]},
    ) as test_client:
...
async def test_group():
    group_config = await policy.add_group_users(
        params=GroupUserReq(group_name="test_group", user_str="test_user")
    )
    yield group_config
...
async def test_policy(test_admin_user, test_action):
    policy_config = await policy.create_policy(
        flow_data=PolicyFlowReq(
            name="test_policy",
            display="test",
            definition=NodeDefinition(
                nodes=[
                    Node(
                        name="test_node",
                        approvers="test_user",
                        approver_type=ApproverType.PEOPLE,
    ...
    await policy.add_associate(
        params=TicketPolicyReq(
    ...
    policy_config = await policy.create_policy(
        flow_data=PolicyFlowReq(
            ...
            definition=NodeDefinition(
                nodes=[
                    Node(
                        name="test_approval_node",
                        approvers=test_group.group_name,
            ...
            definition=NodeDefinition(
                nodes=[
                    Node(
                        name="test_approval_node",
                        approvers="",
                        approver_type=ApproverType.APP_OWNER,
                        node_type=NodeType.APPROVAL,
                    )
                ]
            ),
        ...
        flow_data=PolicyFlowReq(
            ...
            definition=NodeDefinition(
                nodes=[
                    Node(
                        name="test_node1",
                        approvers="admin_user, normal_user",
                        approver_type=ApproverType.PEOPLE,
                        node_type=NodeType.APPROVAL,
                    ),
                    Node(
...
def test_all_policy(
    test_policy,
    test_cc_submitter_policy,
    test_cc_others_policy,
    test_approval_policy_by_group,
    test_approval_policy_by_app,
