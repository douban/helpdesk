...
async def get_policy_associate(
    ...
    if config_type == ConfigType.ticket:
        action_tree_leaf = (
            action_tree.find(target_object)
            if target_object != ""
            else action_tree.first()
        )
...
async def add_associate(params: TicketPolicyReq, _: User = Depends(require_admin)):
    ...
    if not ticket_policy:
        raise HTTPException(
            status_code=500, detail="add ticket policy associate failed"
        )
...
async def update_associate(
    id: int, params: TicketPolicyReq, _: User = Depends(require_admin)
):
    associate = await TicketPolicy.get(id)
    if not associate:
        raise HTTPException(
            status_code=404, detail="ticket and policy associate not found"
        )
...
async def update_group_users(
    id: int, params: GroupUserReq, _: User = Depends(require_admin)
):
