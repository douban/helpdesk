...
async def get_admin_panel_config(
    target_object: str, config_type: str, _: User = Depends(require_admin)
):
    action_tree_leaf = (
        action_tree.find(target_object) if target_object != "" else action_tree.first()
    )
...
async def action(
    target_object: str, request: Request, current_user: User = Depends(get_current_user)
):
...
async def mark_ticket(ticket_id: int, mark: MarkTickets, token: Optional[str] = None):
    ...
    try:
        helpdesk_ticket.annotate(
            execution_status=mark.execution_status, final_exec_status=True
        )
...
async def ticket_op(
    ...
    if ticket.status != "pending":
        raise HTTPException(
            status_code=400, detail="Ticket not pending, can not be operated"
        )
    ...
    if op == "close":
        ...
        if ticket.submitter != current_user.name:
            raise HTTPException(
                status_code=403, detail="Permission denied, only submitter can close"
            )
        ...
        if not id_:
            raise HTTPException(
                status_code=500,
                detail="ticket executed closed but failed to save state",
            )
    ...
    if op == "approve":
        ...
        if ret and "Success" not in msg:
            ...
            if not ticket_id:
                raise HTTPException(
                    status_code=500,
                    detail="Failed to save ticket info when has next approval",
                )
    ...
    if not id_:
        msg = (
            "ticket executed but failed to save state"
            if op == "approve"
            else "Failed to save ticket state"
        )
        raise HTTPException(status_code=500, detail=msg)
    return dict(msg="Success")
...
async def list_ticket(
    ...
    desc: bool = False,
    current_user: User = Depends(get_current_user),
    query_key: Optional[QeuryKey] = None,
    ...
    if current_user.is_admin and show_all:
        ...
        total = await Ticket.count_by_submitter(
            submitter=current_user.name, filter_=filter_
        )
...
async def ticket_result(ticket_id: int, _: User = Depends(get_current_user)):
    ...
    try:
        ...
        if (
            exec_status
            and annotation_execution_status != exec_status
...
async def ticket_result_log(
    ticket_id: int, exec_output_id: str, _: User = Depends(get_current_user)
):
    ticket = await Ticket.get(ticket_id)
    if not ticket:
        raise HTTPException(status_code=404, detail="ticket not found")
